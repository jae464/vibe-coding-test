...
export class ProblemsController {
  ...
  async findAll() {
    ...
    return ApiResponseDto.success(problems, '문제 목록을 성공적으로 조회했습니다.');
  }
  ...
  async findByContest(@Param('contestId') contestId: string) {
    ...
    return ApiResponseDto.success(problems, '대회별 문제 목록을 성공적으로 조회했습니다.');
  }

  @Get(':id')
  async findOne(@Param('id') id: string) {
    const problem = await this.problemsService.findOne(+id);
    return ApiResponseDto.success(problem, '문제 정보를 성공적으로 조회했습니다.');
  }
  ...
  async update(@Param('id') id: string, @Body() updateProblemDto: Partial<CreateProblemDto>) {
    ...
    return ApiResponseDto.success(problem, '문제 정보가 성공적으로 수정되었습니다.');
  }
